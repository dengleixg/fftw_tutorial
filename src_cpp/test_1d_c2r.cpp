...
int test_1d_c2r(int n) {
    ...
    complex<double>* in = (complex<double>*)fftw_alloc_complex(nCplx);
    double *ref_out = fftw_alloc_real(n);
    double *fftw_out = fftw_alloc_real(n);

    fftw_plan p = fftw_plan_dft_c2r_1d(n, 
        reinterpret_cast<fftw_complex*>(in), fftw_out, FFTW_ESTIMATE);

    // fill the input array with random data
    fill_random_1d_cplx(nCplx, reinterpret_cast<fftw_complex*>(in));
    ...
    for (int k = 0; k < n; ++k) {
        ...
        if (n % 2 == 0) {
            ...
            ref_out[k] += real(in[nCplx - 1]) * cos(phi);
        ...
        for (int j = 1; j < loopEnd; ++j) {
            ...
            _real = real(in[j]) * cos(phi) - imag(in[j]) * sin(phi);
            ref_out[k] += 2.0 * _real;
        }
    }
